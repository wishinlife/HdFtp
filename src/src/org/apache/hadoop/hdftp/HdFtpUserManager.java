 ...
 */
public class HdFtpUserManager extends AbstractUserManager {

	private static final transient Logger LOG = LoggerFactory.getLogger(HdFtpUserManager.class);
	 ...
	 */
	public HdFtpUserManager(String adminName, PasswordEncryptor passwordEncryptor) {
	 ...
	 * Constructor - Initialize HdfsUsermanager
	 */
	public HdFtpUserManager(String adminName, PasswordEncryptor passwordEncryptor, File userDataFile) {
		super(adminName, passwordEncryptor);
		//this.adminName = adminName;
		this.passwordEncryptor = passwordEncryptor;
	...
	/**
	 * Check whether the user data file is set
	 */
	private void checkConfigure() {
		if (userDataFile == null) {
			throw new IllegalStateException("Must be set user data file");
		}
	}
	...
	public synchronized void save(User user) throws FtpException {
		...
		userDataProp.setProperty(thisPrefix + ATTR_PASSWORD, usr.getPassword());
		...
		userDataProp.setProperty(thisPrefix + ATTR_GROUP, usr.getGroup());
	...
	public synchronized HdFtpUser getUserByName(String userName) {
		...
		user.setHomeDirectory(userDataProp.getString(baseKey + ATTR_HOME, "/"));
		user.setGroup(userDataProp.getString(baseKey + ATTR_GROUP, "ftpusers"));
